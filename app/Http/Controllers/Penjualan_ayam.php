...
class Penjualan_ayam extends Controller
{
    ...
    public function save_jurnal(Request $r)
    {
        ...
        DB::table('tb_jurnal')->where('no_nota', 'A-' . $no_nota)->delete();
    ...
    public function delete_a(Request $r)
    {
        DB::table('invoice_ayam')->where('no_nota', $r->nota)->delete();
        DB::table('tb_jurnal')->where('no_nota', 'A-' . $r->nota)->delete();
        return redirect()->route("pen_ayam")->with('sukses', 'Data berhasil di hapus');
    }

    public function edit_invoice_ayam(Request $r)
    {
        $ayam = DB::table('invoice_ayam')->where('no_nota', $r->nota)->first();
        $data = [
            'title' => 'Penjualan Ayam',
            'costumer' => DB::table('tb_post_center')->where('id_akun', '19')->get(),
            'ayam' => $ayam
        ...
        return view('penjualan_ayam.edit', $data);
    }

    public function edit_ayam(Request $r)
    {
        $tgl = $r->tgl;
        $id_post = $r->id_post;
        $driver = $r->driver;
        ...
        DB::table('invoice_ayam')->where('no_nota', $no_nota)->update($data);
        return redirect()->route("nota_ayam", ['nota' => $no_nota])->with('sukses', 'Data berhasil di input');
    }
}
