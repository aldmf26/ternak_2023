...
class Penjualan_pupuk extends Controller
{
    ...
    public function add_pupuk(Request $r)
    {
        $data = [
            'title' => 'Penjualan Pupuk',
            'costumer' => DB::table('tb_post_center')->where('id_akun', '20')->get(),
        ];
    ...
    public function save_jurnal_pupuk(Request $r)
    {
        ...
        DB::table('tb_jurnal')->where('no_nota', 'P-' . $no_nota)->delete();
    ...
    public function delete_pupuk(Request $r)
    {
        DB::table('invoice_pupuk')->where('no_nota', $r->nota)->delete();
        DB::table('tb_jurnal')->where('no_nota', 'P-' . $r->nota)->delete();
        return redirect()->route("pen_pupuk")->with('sukses', 'Data berhasil di hapus');
    }

    public function edit_i_pupuk(Request $r)
    {
        $pupuk = DB::table('invoice_pupuk')->where('no_nota', $r->nota)->first();
        $data = [
            'title' => 'Penjualan Pupuk',
            'costumer' => DB::table('tb_post_center')->where('id_akun', '20')->get(),
            'pupuk' => $pupuk
        ...
        return view('penjualan_pupuk.edit', $data);
    }

    public function edit_pupuk(Request $r)
    {
        $tgl = $r->tgl;
        $id_post = $r->id_post;
        $driver = $r->driver;
        ...
        DB::table('invoice_pupuk')->where('no_nota', $no_nota)->update($data);
        return redirect()->route("nota_pupuk", ['nota' => $no_nota])->with('sukses', 'Data berhasil di input');
    }
}
