...
class Setoran_telur extends Controller
{
    ...
    public function save_perencanaan_telur(Request $r)
    {
        ...
        for ($x = 0; $x < count($no_nota); $x++) {
            ...
            if ($id_akun[$x] == '33') {
                ...
                $not = $no_nota[$x];
                $telur = DB::selectOne("SELECT a.no_nota FROM invoice_telur as a where concat('T-',a.no_nota) = '$not'");
                $data = [
                    'setor' => 'Y',
                    'nota_setor' => 'T' . $urt
                ];
                DB::table('invoice_telur')->where('no_nota', $telur->no_nota)->update($data);
            } else {
                $data = [
                    'no_nota' => $no_nota[$x],
                    'nota_setor' => 'B' . $urt2,
    ...
    public function cancel_perencanaan_st(Request $r)
    {
        ...
        DB::table('invoice_telur')->where('nota_setor', $r->nota)->update(['setor' => 'T', 'nota_setor' => '']);
